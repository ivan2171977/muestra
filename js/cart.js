// Mantiene la maqueta del carrito aunque esté vacío
function renderCart() {
  ...
  if (!items.length) {
    tbody.innerHTML = `
      <tr>
        <td>
          <div style="font-weight:600;color:var(--primary)">—</div>
          <div class="small">Talla: —</div>
        </td>
        <td class="right">—</td>
        <td class="right">
  ...
  tbody.querySelectorAll(".rmItem").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      const arr = readCart(); arr.splice(idx,1); writeCart(arr); renderCart(); updateCartBadge();
    });
  });
}
...
function doPay() {
  ...
  setTimeout(()=> { alert("¡Gracias! Tu pedido fue preparado en tu correo. El carrito se limpió."); window.location.href = "catalogo.html"; }, 300);
}
